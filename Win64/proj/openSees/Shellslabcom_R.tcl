# units: m,N
# A square 3 m x 3 m slab with 0.05 m deep ribs and 0.05 m deep flat; the bottom is 1 mm thick steel deck. 
# 30 mm clear concrete cover from top, and 335.1 mm2/m reinforcement in x and y directions.
wipe;	
...
nDMaterial PlateRebarThermal 10 9 90;
...
section LayeredShellThermal  11 -offset $offset 10  10  0.01  10 0.01 10 0.01 10  0.01  10  0.01 4  0.01  4  0.01  4 0.01 4 0.01  4 0.01;
section CompositeShellThermal 10 2 0.5 4 0.5; 
section CompositeShellThermal 12 2 0.5 11 0.5; 
...
if {$analysis == "M3"} {
    ...
    for {set j 1} {$j <= $ny} {incr j 1} {
        for {set i 1} {$i <= $nx} {incr i 1} {
		  ...
		  set node4 [expr $node1-1]
		  set elementID $node4
		  element ShellNLDKGQThermal $elementID $node1 $node2 $node3 $node4 12
		}
	}
}  
...
recorder Node -file ShellData/DFreeSlabxT.out -time -nodeRange [expr ($ny/2+1)*100+1] [expr ($ny/2+1)*100+($nx+1)] -dof 1  disp; #nodes in Y=1.5;
recorder Node -file ShellData/DFreeSlabyT.out -time -nodeRange [expr ($ny/2+1)*100+1] [expr ($ny/2+1)*100+($nx+1)] -dof 2 disp; #nodes in Y=1.5;
recorder Node -file ShellData/DFreeSlabzT.out -time -node [expr ($ny/2+1)*100+($nx+1)] -dof 3 disp; #nodes in x=3.0, y=1.5
recorder Node -file ShellData/DFreeSlabx.out -time -node [expr ($ny/2+1)*100+($nx+1)] -dof 1  disp; #nodes in Y=1.5;
recorder Node -file ShellData/DFreemidNodeU3.out -time -node $midNode -dof 3 disp; #center node

recorder Node -file ShellData/DFreeSlabzTUP.out -time -nodeRange [expr 1*100+1] [expr 1*100+($nx+1)] -dof 3 disp; #nodes in Y=0
recorder Node -file ShellData/DFreeSlabzTDo.out -time -nodeRange [expr ($ny+1)*100+1] [expr ($ny+1)*100+($nx+1)] -dof 3 disp; #nodes in Y=3
