...
uniaxialMaterial ElasticThermal 2 2e10 1.4e-5;
...
uniaxialMaterial ConcreteECThermal 4 $fpc $epsc0 $fpcu $epsU $lambda $ft $Ets
...
section fiberSecThermal $secTag {
	for {set fiberID 1} {$fiberID<=$NumFibers} {incr fiberID} {
		set fiberLocy [expr ($d/$NumFibers)*($fiberID-0.5)];
		set fiberArea [expr ($d/$NumFibers)*$b*0.5];
		fiber $fiberLocy -10 $fiberArea 2;
		fiber $fiberLocy 10 $fiberArea 2;
		puts "$fiberLocy, $fiberArea";
		
		set fiberLocy1 [expr -($d/$NumFibers)*($fiberID-0.5)];
		set fiberArea1 [expr ($d/$NumFibers)*$b*0.25];
		fiber $fiberLocy1 -10 $fiberArea1 2;
		fiber $fiberLocy1 10 $fiberArea1 2;
		puts "$fiberLocy1, $fiberArea1";
	}
	#fiber -25 -50 56.5 3;
	#fiber -25 50 56.5 3;	
	#fiber 30 -50 78.54 3;
...
recorder Node -file WrapperData/N_End_DOF.out -time -node $EndNode -dof 2 disp;
...
recorder Element -file WrapperData/EleSecbotT.out -time -ele section 1  fiber [expr -($d/$NumFibers)*($NumFibers-0.5)] 10 TempElong;
recorder Element -file WrapperData/EleSecbotss.out -time -ele 1 section 1  fiber [expr -($d/$NumFibers)*($NumFibers-0.5)]  10 stressStrainTangent;
recorder Element -file WrapperData/EleSectopT.out -time -ele section 1  fiber [expr ($d/$NumFibers)*($NumFibers-0.5)] 10 TempElong;
recorder Element -file WrapperData/EleSectopss.out -time -ele 1 section 1  fiber [expr ($d/$NumFibers)*($NumFibers-0.5)]  10 stressStrainTangent;
...
eleLoad -range 1 $NumEles -type -beamThermal 1000 -$HalfD 200 $HalfD
...
numberer Plain;
system BandGeneral;
test NormUnbalance 1.0e-3 500 1;
algorithm Newton;
