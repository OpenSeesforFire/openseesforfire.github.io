...
for {set yid 0} {$yid <= $ny} {incr yid} {
for {set xid 0} {$xid <= $nx} {incr xid} {
set locX [expr $xid*$slabL/$nx];
set locY [expr $yid*$slabB/$ny];
set NodeTag [expr $xid+($nx+1)*$yid+1];
node $NodeTag $locX $locY 0;
}
}

for {set yid 0} {$yid < $ny} {incr yid} {
for {set xid 0} {$xid < $nx} {incr xid} {
set Tag1 [expr $xid+($nx+1)*$yid+1];
set Tag2 [expr $xid+($nx+1)*$yid+2];
set Tag3 [expr $xid+($nx+1)*($yid+1)+2];
set Tag4 [expr $xid+($nx+1)*($yid+1)+1];
set EleTag [expr $xid+$nx*$yid+1];
element ShellNLComThermal $EleTag $Tag1 $Tag2 $Tag3 $Tag4 20;
}
}
...
recorder Element -file ShellDataLim12/EleForceSec1Temp.out -time -ele $MidEle   material 1 comp 1 fiber 1 TempAndElong;
...
pattern Plain 3 Linear {
	...
	#The 1st source is assigned to the subsection 1 in the composite section;
}
