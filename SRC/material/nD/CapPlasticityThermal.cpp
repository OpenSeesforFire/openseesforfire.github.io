...
//#include <math.h>
#include <CapPlasticityThermal.h>
...
Vector CapPlasticityThermal::tempVector(6);
Matrix CapPlasticityThermal::tempMatrix(6,6);  

void *
OPS_CapPlasticityThermal(void) {
  int tag;
  int ndm =3;
  double rho = 0.0;
  double G = 1.0e10;
  ...
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid double values: nDMaterial CapPlasticityThermal " << tag << endln;
    return 0;
  }  
  ...
  NDMaterial *theMaterial = new CapPlasticityThermal(  tag,
						G,
						K,
						rho,
						X,
...
CapPlasticityThermal::CapPlasticityThermal( int    pTag,
			      double pG,
			      double pK,
			      double pRho,
			      double pX,
...
: NDMaterial(pTag,ND_TAG_CapPlasticityThermal),
  CStrain(6),
  CPlastStrain(6),
  CStress(6),
  strain(6), 
...
CapPlasticityThermal::CapPlasticityThermal( const CapPlasticityThermal & a)
  : NDMaterial(a.getTag(),ND_TAG_CapPlasticityThermal),CStrain(6),CPlastStrain(6),CStress(6),
...
CapPlasticityThermal::~CapPlasticityThermal( ) {
  return;
  
};

double CapPlasticityThermal::getRho(void) {
  return rho;
};

int CapPlasticityThermal::setTrialStrain(const Vector &pStrain) {  // strain from element is eng. strain!
  ...
  if (ndm==3 && pStrain.Size()==6) 
    ...
    opserr << "Fatal:CapPlasticityThermal:: Material dimension is: " << ndm << endln;
    opserr << "But strain vector size is: " << pStrain.Size() << endln;
    //exit(-1);
	opserr<< "Warning: errors in CapPlasticityThermal::setTrialStrain"<<endln;
  }
...
int CapPlasticityThermal::setTrialStrain(const Vector &pStrain, const Vector &r) {
  
  return setTrialStrain(pStrain);
  
};

int CapPlasticityThermal::setTrialStrainIncr(const Vector &pStrainRate) {
  ...
  if (ndm==3 && pStrainRate.Size()==6) 
	...
	opserr<< "Warning: errors in CapPlasticityThermal::setTrialStrainIncr"<<endln;	
   // exit(-1);
  }
...
int CapPlasticityThermal::setTrialStrainIncr(const Vector &pStrainRate, const Vector &r) {
  return setTrialStrainIncr(pStrainRate);
};



const Vector & CapPlasticityThermal::getStrain(void) {
...
int CapPlasticityThermal::commitState(void)  {
  CStrain = strain;
  CStress = stress;
...
int CapPlasticityThermal::revertToLastCommit(void)  {
  return 0;
};

int CapPlasticityThermal::revertToStart(void)  {
...
NDMaterial * CapPlasticityThermal::getCopy(void)  {
  
  CapPlasticityThermal * copy = new CapPlasticityThermal(*this);
  return copy;
  
};

NDMaterial * CapPlasticityThermal::getCopy(const char *code)  {
...
const char * CapPlasticityThermal::getType(void) const  {
  return (ndm == 2) ? "PlaneStrain" : "3DThermal";
};

int CapPlasticityThermal::getOrder(void) const  {
  return (ndm == 2) ? 3 : 6;
};

int CapPlasticityThermal::sendSelf(int commitTag, Channel &theChannel)  {return 0;};

int CapPlasticityThermal::recvSelf(int commitTag, Channel &theChannel,  
			    FEM_ObjectBroker &theBroker )  {return 0;};

//Response * CapPlasticityThermal::setResponse (const char **argv, int argc, Information &matInformation)  {
  
Response*
CapPlasticityThermal::setResponse (const char **argv, int argc, OPS_Stream &output) {
...
int CapPlasticityThermal::getResponse (int responseID, Information &matInfo)  {
...
void CapPlasticityThermal::Print(OPS_Stream &s, int flag)  {return;};

// --------------

double CapPlasticityThermal::CapBoundL(double k){
  if (k>0) 
    return k; 
  return 0;
};
...
// --------------
double CapPlasticityThermal::CapBoundX(double k){  // function of X
  return k+R*failureEnvelop(k);
}; 

// --------------
double CapPlasticityThermal::failureEnvelop(double I){
  
  return alpha - lambda*exp(-beta*I)+theta*I; 
};

// --------------
double CapPlasticityThermal::CapSurface(double normS,double I, double k){  // Fc
...
// --------------
double CapPlasticityThermal::failureEnvelopDeriv(double I){
  return lambda*beta*exp(-beta*I)+theta;
};

// --------------
double CapPlasticityThermal::hardeningParameter_H(double k2, double k1){

  return W*(exp(-D*CapBoundX(k1))-exp(-D*CapBoundX(k2)));
}; 

int CapPlasticityThermal::findMode(double normS, double I1, double k){
...
// --------------
double CapPlasticityThermal::Newton_k(double tol, int mode /*, double normS, double I1, double k */){
  ...
  if ( mode == 0){
    ...
    if (fabs(f) > tol) {
      opserr<< "Fatal : Newton algorithm does not converge, in CapPlasticityThermal, mode =0! ";
	 // opserr<< "Warning: errors in CapPlasticityThermal::Newton_k"<<<" "<"mode="<<mode<<endln;	
     // exit(-1);
    }
    solution = k;
    ...
    if (fabs(f) > tol) {
      opserr<< " Newton algorithm does not converge, in CapPlasticityThermal, mode = "<<" "<<"mode"<<endln;
      //opserr<< "Warning: errors in CapPlasticityThermal::setTrialStrainIncr"<<endln;	
    }
    solution = k;
 ...
 if (solution <0 ) {
   opserr<<"Warning: CapPlasticityThermal:: Newton_k, solution <0! mode is " <<mode<< "! k should be adjusted to CHardening_k! "endln;    // --- April 2013. 
...
double CapPlasticityThermal::Newton_I1(double tol, int mode, double normS, double I1_trial){
  ...
  if ( mode == 5){   // get I1, that is the first step of mode 5
    ...
    if (fabs(f) > relative_tol) {
      opserr<< "mode =5. Newton algorithm does not converge, in CapPlasticityThermal, Newton_I1 mode =5! ";
      //exit(-1);
    }
    solution = I1;
...
double CapPlasticityThermal::Bisection(double tol, double normS, double I1_trial ){
  ...
  if (fabs(f) > relative_tol) {
    ...
    opserr<< "Warning: Newton can not converge in CapPlasticityThermal::Bisection"<<endln;
    //exit(-1);
  }
  double x_up = k;
  ...
  if (f_low*f_up>0 ) {
    opserr <<"Warning2: Bisection can not converge in  CapPlasticityThermal::Bisection! "<<endln;
    //exit(-1);
  ...
  if (fabs(f) > relative_tol) {
    opserr<< "Warning3:No convergence in CapPlasticityThermal::Bisection"<<endln;
    //exit(-1); 
  }
  
  flag =1; 
 if (k <0 ) {
	opserr<<"Fatal: CapPlasticityThermal:: Bisection, k <0! mode is 3 ! k is adjusted to CHardening_k !!!!! "endln;    // --- April 2013. 
	k = CHardening_k;
...
const Matrix & CapPlasticityThermal::getTangent(void) {
...
const Matrix & CapPlasticityThermal::getInitialTangent(void) {
  return getTangent();
};

const Vector & CapPlasticityThermal::getStress(void) {
...
Matrix & CapPlasticityThermal::dF2dSigma ( int mode){ // the returned matrix has been consistent with notation (i.e., last 3 columns multiplied by 2) 
  ...
  if (mode ==5) {  // f1
    ...
    opserr<< "warning: CapPlasticityThermal::dF2dSigma() should not be called! mode is "<< mode<<endln;
    
    
  }

/*// ---- debug only ----------
   opserr<<" CapPlasticityThermal::dF2dSigma(): " <<endln;
   for (  i =0; i<6; i++) {
		for (int j=0; j<6; j++)
...
Vector & CapPlasticityThermal::dFdSigma (int mode){
  ...
  if( mode ==5) {
    ...
    opserr<< "warning: CapPlasticityThermal::dFdSigma() should not be called! mode is "<< mode<<endln;
    
  }
  return tempVector;
};

double CapPlasticityThermal::dFdk (int OrderOfDerivative){
...
Vector CapPlasticityThermal::dF2dSigmadk ( void){
...
double CapPlasticityThermal::dHdk(double hardening_k){
...
double 
CapPlasticityThermal::tripleTensorProduct (Vector &A, Matrix &B, Vector &C){ // result = A:B:C

	if ((A.Size() !=6) ||(C.Size() !=6) || (B.noCols() !=6) || (B.noRows() !=6)) {
		opserr<<"Fatal: CapPlasticityThermal::tripleTensorProduce() size does not match! "<<endln;
		exit(-1);
	}
...
double 
CapPlasticityThermal::dFdIdk(void){
...
int 
CapPlasticityThermal::computeConsistentTangent(double deltaGammar1, double deltaGammar2, double deltaGammar3, int mode){
...
///////////////////////// add sensitivity ///////////////////////////////////
int 
CapPlasticityThermal::setParameter (const char **argv, 
...
{	if (argc < 1)
	...
	if (strcmp(argv[0],"T") == 0) {
		...
		opserr << "WARNING: Could not set parameter in CapPlasticityThermal. " << endln;
                
	return -1;
}
	
int 
CapPlasticityThermal::updateParameter(int passedParameterID, 
												 Information &info){
...
int CapPlasticityThermal::activateParameter(int passedParameterID){
...
const Vector & 
CapPlasticityThermal::getStressSensitivity  (int gradNumber, bool conditional){
...
const Vector& CapPlasticityThermal::getTempAndElong( ){
  static Vector TempElong(2);
  TempElong.Zero();
