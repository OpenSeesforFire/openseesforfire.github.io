...
// Modified for SIF modelling by Liming Jiang [http://openseesforfire.github.io] 
 ...
 #include <CDPPlaneStressThermal.h>           
 #include <Channel.h>
 #include <cmath>
...
void *
OPS_CDPPlaneStressThermal(void)
{
  ...
  if (numArgs < 5 || numArgs > 9) {
    opserr << "Want: nDMaterial CDPPlaneStressThermal $tag $E $nu $ft $fc <$beta $Ap $An $Bn>\n";
    return 0;
  }
  ...
  theMaterial = new CDPPlaneStressThermal(iData[0],
 ...
 CDPPlaneStressThermal::CDPPlaneStressThermal(int tag,
						  double _e, 
						  double _nu, 
						  double _ft,
						  double _fc, 
 ...
 CDPPlaneStressThermal::CDPPlaneStressThermal()
   :NDMaterial (0, 0),
    stress(3),strain(3), Cstress(3), Cstrain(3)
 {
 ...
 int
 CDPPlaneStressThermal::setTrialStrain(Vector const&v1, Vector const&v2){
   return this->setTrialStrain(v1);
 }

 int
 CDPPlaneStressThermal::setTrialStrain (const Vector &strain)
 {
   eps[0] = strain(0);
   eps[1] = strain(1);
   eps[2] = strain(2);
   ...
   double sigpmp;
   double sigmaC;
   double lamda;   //gamma in the reference paper (Lee&Fenves,2001)
   double xi;

   StrsPr(sige_tr, fi, sigPr1, sigPr2, strsMises);
   ...
   //determine beta 
   beta = fc / ft*(alpha - 1) - (1 + alpha);
   //Yield surface
   fyield = (alpha*(sigPr1 + sigPr2) + strsMises + beta*sigpmp) / (1 - alpha) - sigmaC;

   if (fyield > tol) {
		
	   

	   //residual of damage variable k
	   double Qnorm;
	   double normX;
	   double eta;
	   double norms;
	   ...
	   norms = sqrt(2.0/3.0*(sigPr1*sigPr1+sigPr2*sigPr2-sigPr1*sigPr2));
	   normX = xi*(norms+2*G*lamda)
	   
	   lamda = alpha*(sigPr1 + sigPr2)+ sqrt(3.0/2.0)*normdevs+ ;
   ...
   opserr <<  "CDPPlaneStressThermal::sTS -12\n";     
   ...
   //   opserr <<  "CDPPlaneStressThermal::sTS -18\n";         
   return 0;
 }


int
CDPPlaneStressThermal::setTrialStrainIncr (const Vector &dStrain)
{
  strain += dStrain;
...
int
CDPPlaneStressThermal::setTrialStrainIncr (const Vector &dStrain, const Vector &rate)
{
  strain += dStrain;
  this->setTrialStrain(strain);
  return 0;
...
const Matrix&
CDPPlaneStressThermal::getTangent (void)
{
  return Ce;
}

const Matrix&
CDPPlaneStressThermal::getInitialTangent (void)
{
  return Ce0;
}

const Vector&
CDPPlaneStressThermal::getStress (void)
{
  return stress;
}

const Vector&
CDPPlaneStressThermal::getStrain (void)
{
  return strain;
}

int
CDPPlaneStressThermal::commitState (void)
{
  CeCommitted = Ce;
...
int
CDPPlaneStressThermal::revertToLastCommit (void)
{
  Ce = CeCommitted; 
...
int
CDPPlaneStressThermal::revertToStart (void)
{
  return 0;
}

NDMaterial*
CDPPlaneStressThermal::getCopy(const char *type)
{
  if (strcmp(type,"PlaneStress") == 0 || strcmp(type,"PlaneStress2D") == 0) {
    CDPPlaneStressThermal *theCopy =
      new CDPPlaneStressThermal (this->getTag(), E, nu, ft, fc, beta, Ap, An, Bn);
...
NDMaterial*
CDPPlaneStressThermal::getCopy (void)
{
  CDPPlaneStressThermal *theCopy =
    new CDPPlaneStressThermal (this->getTag(), E, nu, ft, fc, beta, Ap, An, Bn);
...
const char*
CDPPlaneStressThermal::getType (void) const
{
  return "PlaneStress2d";
}

int
CDPPlaneStressThermal::getOrder (void) const
{
  return 6;
}
...
int 
CDPPlaneStressThermal::sendSelf(int commitTag, Channel &theChannel)
{
  static Vector data(10);

  int res = theChannel.sendVector(this->getDbTag(), commitTag, data);
  if (res < 0) {
    opserr << "CDPPlaneStressThermal::sendSelf -- could not send Vector\n";
    return res;
  }
...
int 
CDPPlaneStressThermal::recvSelf(int commitTag, Channel &theChannel, 
...
void 
CDPPlaneStressThermal::Print(OPS_Stream &s, int flag) {
  opserr << "CDPPlaneStressThermal: " << this->getTag();
  opserr << "strain: " << eps;
  opserr << "strain: " << sig;
...
const Vector&
CDPPlaneStressThermal::getTempAndElong( void)
{
    return TempAndElong;
}
...
double
CDPPlaneStressThermal::setThermalTangentAndElongation(double &tempT, double&ET, double&Elong)
{
...
void 
CDPPlaneStressThermal::StrsPr(const Vector&sig, double &fi, double &sigPr1, double &sigPr2, double &strsMises)
{
	double sigpr1, sigpr2;
