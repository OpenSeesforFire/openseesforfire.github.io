...
cStrain01(0),cStrain1(0),cStrain02(0),cStrain2(0),
TempAndElong(2),epst(0)
{ }
...
TempAndElong(2),epst(0),crack1(0),Cps1(0),Cps2(0),crack2(0)
{
  fc = fc0;
  ft = ft0;
  E=E0;
...
ConcreteSThermal::commitState( ) 
{
  ...
  Cps2 = sqrt(sigd * sigd + stress(2) * stress(2));
  Cps1 = sigm + Cps2;
  Cps2 = sigm - Cps2;
...
ConcreteSThermal::setThermalTangentAndElongation(double &tempT, double&ET, double&Elong)
{
  ...
  if (Temp <= 80) {
	...
	epst = ft0/Es0*(1.0 - 1.0*(Temp -80)/500);
...
//  if (psmax < ftmin) psmax = ftmin;
  ...
  if(ps1>ft) crack1 =true;

  cStrain1 = cStrain01 + yieldFunc /E;

  if (ps1>0&&yieldFunc > 0.0&& crack1)
  {
    //ps1 = ft + Ep * cStrain;
	double eStrain = ft/E;
	if(cStrain1<epst)
		ps1=(ft)*(epst-cStrain1)/epst;
	else
		ps1=0.001*ft;

	//tangent*=(-0.1)
  }
  TempAndElong(0)= cStrain1;
  TempAndElong(1)= yieldFunc;

  yieldFunc = ps2 - Cps2;

   if(ps2>ft) crack2 =true;

  cStrain2 = cStrain02 + yieldFunc /E;

   if (ps2>0&&yieldFunc > 0.0&& crack2)
  {
	

    //ps1 = ft + Ep * cStrain;
	if(cStrain2<epst)
		ps2=(ft)*(epst-cStrain2)/epst;
	else
		ps2=0.001*ft;
