...
// $Source: /usr/local/cvs/OpenSees/SRC/element/shell/ShellMITC4GNLThermal.cpp,v $
...
#include <ShellMITC4GNLThermal.h>
#include <R3vectors.h>
#include <Renderer.h>
...
//static data
Matrix  ShellMITC4GNLThermal::stiff(24,24) ;
Vector  ShellMITC4GNLThermal::resid(24) ;
Matrix  ShellMITC4GNLThermal::mass(24,24) ;

//intialize pointers to zero using intialization list 
Matrix**  ShellMITC4GNLThermal::GammaB1pointer = 0 ;
Matrix**  ShellMITC4GNLThermal::GammaD1pointer = 0 ;
Matrix**  ShellMITC4GNLThermal::GammaA2pointer = 0 ;
Matrix**  ShellMITC4GNLThermal::GammaC2pointer = 0 ; 
Matrix**  ShellMITC4GNLThermal::Bhat           = 0 ;
    
//quadrature data
const double  ShellMITC4GNLThermal::root3 = sqrt(3.0) ;
const double  ShellMITC4GNLThermal::one_over_root3 = 1.0 / root3 ;

double ShellMITC4GNLThermal::sg[4] ;
double ShellMITC4GNLThermal::tg[4] ;
double ShellMITC4GNLThermal::wg[4] ;


#define min(a,b) ( (a)<(b) ? (a):(b) )

static int numShellMITC4GNLThermal = 0;

void *
OPS_NewShellMITC4GNLThermal(void)
{
  if (numShellMITC4GNLThermal == 0) {
    opserr << "Using ShellMITC4GNLThermal - Developed by: Leopoldo Tesser, Diego A. Talledo, Veronique Le Corvec\n";
    numShellMITC4GNLThermal++;
  }
  ...
  if (numArgs < 6) {
    opserr << "Want: element ShellMITC4GNLThermal $tag $iNode $jNoe $kNode $lNode $secTag";
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid integer tag: element ShellMITC4GNLThermal \n";
    return 0;
  }
  ...
  if (theSection == 0) {
    opserr << "ERROR:  element ShellMITC4GNLThermal " << iData[0] << "section " << iData[5] << " not found\n";
    return 0;
  }
  
  theElement = new ShellMITC4GNLThermal(iData[0], iData[1], iData[2], iData[3],
			      iData[4], *theSection);

  return theElement;
}
/*
const double  ShellMITC4GNLThermal::sg[] = { -one_over_root3,  
				one_over_root3, 
				one_over_root3, 
	                       -one_over_root3 } ;

const double  ShellMITC4GNLThermal::tg[] = { -one_over_root3, 
                               -one_over_root3, 
                                one_over_root3,  
                                one_over_root3 } ;

const double  ShellMITC4GNLThermal::wg[] = { 1.0, 1.0, 1.0, 1.0 } ;
*/
  

//null constructor
ShellMITC4GNLThermal::ShellMITC4GNLThermal( ) :
Element( 0, ELE_TAG_ShellMITC4GNLThermal ),
...
//*********************************************************************
//full constructor
ShellMITC4GNLThermal::ShellMITC4GNLThermal(  int tag, 
                         int node1,
                         int node2,
   	                 int node3,
                         int node4,
	                 SectionForceDeformation &theMaterial ) :
Element( tag, ELE_TAG_ShellMITC4GNLThermal ),
connectedExternalNodes(4), load(0), Ki(0)
{
  int i;
  ...
  for ( i = 0 ;  i < 4; i++ ) {
	...
	opserr << "ShellMITC4GNLThermal::constructor - failed to get a material of type: ShellSection\n";
      } //end if
...
//set domain
void  ShellMITC4GNLThermal::setDomain( Domain *theDomain ) 
{  
  int i, j ;
  static Vector eig(3) ;
  ...
  for ( i = 0; i < 4; i++ ) {
     ...
     if (nodePointers[i] == 0) {
       opserr << "ShellMITC4GNLThermal::setDomain - no node " << connectedExternalNodes(i);
...
int  ShellMITC4GNLThermal::getNumExternalNodes( ) const
{
  return 4 ;
} 
...
const ID&  ShellMITC4GNLThermal::getExternalNodes( ) 
{
...
Node **
ShellMITC4GNLThermal::getNodePtrs(void) 
{
  return nodePointers;
} 

//return number of dofs
int  ShellMITC4GNLThermal::getNumDOF( ) 
{
  return 24 ;
}
...
//commit state
int  ShellMITC4GNLThermal::commitState( )
{
  int success = 0 ;

  // call element commitState to do any base class stuff
  if ((success = this->Element::commitState()) != 0) {
    opserr << "ShellMITC4GNLThermal::commitState () - failed in base class";
  }    
...
int  ShellMITC4GNLThermal::revertToLastCommit( ) 
{
  int i ;
  int success = 0 ;
...
//revert to start 
int  ShellMITC4GNLThermal::revertToStart( ) 
{
  int i ;
  int success = 0 ;
...
void  ShellMITC4GNLThermal::Print( OPS_Stream &s, int flag )
{
  if (flag == -1) {
...
Response*
ShellMITC4GNLThermal::setResponse(const char **argv, int argc, OPS_Stream &output)
{
  Response *theResponse = 0;

  output.tag("ElementOutput");
  output.attr("eleType", "ShellMITC4GNLThermal");
  ...
  if (strcmp(argv[0],"force") == 0 || strcmp(argv[0],"forces") == 0 ||
    ...
    if (argc < 2) {
      opserr << "ShellMITC4GNLThermal::setResponse() - need to specify more data\n";
      return 0;
    }
...
int
ShellMITC4GNLThermal::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
const Matrix&  ShellMITC4GNLThermal::getTangentStiff( ) 
{
...
const Matrix&  ShellMITC4GNLThermal::getInitialStiff( ) 
{
  if (Ki != 0)
    return *Ki;
...
//return mass matrix
const Matrix&  ShellMITC4GNLThermal::getMass( ) 
{
  int tangFlag = 1 ;
...
void  ShellMITC4GNLThermal::zeroLoad( )
{
  if (load != 0)
    load->Zero();
...
int 
ShellMITC4GNLThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
 ...
 //J.Jiang third try
  ...
  residThermal[18] = -ThermalLoad[0]*L23; residThermal[19] = ThermalLoad[0]*L12;residThermal[21] = -ThermalLoad[1]*L23;residThermal[22] = -ThermalLoad[1]*L12;
	 ...
	 for(int i =0; i<9;i++){
		 if(data0(2*i+1)-data1(2*i+1)>1e-8||data0(2*i+1)-data1(2*i+1)<-1e-8){
			 opserr<<"Warning:The NodalThermalAction in ShellMITC4GNLThermal "<<this->getTag()
  ...
  // Loop over the integration points
	...
	for (int j=0; j<4; j++)  {
     ...
     const Vector &s = materialPointers[j]->getTemperatureStress(dataMixV);
     ThermalLoad[0] +=shapefn2d(xi, eta, j+1)*s(0);
	 ThermalLoad[1] +=shapefn2d(xi, eta, j+1)*s(1);
      if (s == 0) {
  ...
  double L12,L23;
  L12 = fabs(theNode2Crds[0]-theNode1Crds[0])/2;
  L23 = fabs(theNode3Crds[1]-theNode2Crds[1])/2;
  ...
  //end of thermalActionWrapper
    ...
    opserr << "ShellMITC4GNLThermal::ShellMITC4GNLThermal -- load type unknown for element with tag: "
	   << this->getTag() << "ShellMITC4GNLThermal::addLoad()\n"; 
  ...
  //opserr << "ShellMITC4Thermal::addLoad - load type unknown for ele with tag: " << this->getTag() << endln;
  //return -1;
}
...
int 
ShellMITC4GNLThermal::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1 ;
...
//get residual
const Vector&  ShellMITC4GNLThermal::getResistingForce( ) 
{
...
const Vector&  ShellMITC4GNLThermal::getResistingForceIncInertia( )
{
...
void   
ShellMITC4GNLThermal::formInertiaTerms( int tangFlag ) 
{
...
//J.Jiang add to update
int
ShellMITC4GNLThermal::update(void)
{
...
void  
ShellMITC4GNLThermal::formResidAndTangent( int tang_flag ) 
{
  //
  //  six(6) nodal dof's ordered :
  //
...
void   
ShellMITC4GNLThermal::computeBasis( ) 
{
...
double*
ShellMITC4GNLThermal::computeBdrill( int node, const double shp[3][4] )
{
...
void   
ShellMITC4GNLThermal::computeGamma( const double xl[2][4], const Matrix &J )
{
...
const Matrix&  
ShellMITC4GNLThermal::assembleB( const Matrix &Bmembrane,
                               const Matrix &Bbend, 
                               const Matrix &Bshear ) 
{
...
const Matrix&   
ShellMITC4GNLThermal::computeBmembrane( int node, const double shp[3][4] ) 
{
...
const Matrix&   
ShellMITC4GNLThermal::computeBbend( int node, const double shp[3][4] )
{
...
const Matrix&  
ShellMITC4GNLThermal::computeBbarShear( int node, double L1, double L2,
			      const Matrix &Jinv ) 
{
...
const Matrix&  
ShellMITC4GNLThermal::computeBshear( int node, const double shp[3][4] )
{
...
void   
ShellMITC4GNLThermal::computeJacobian( double L1, double L2,
				    const double x[2][4], 
                                    Matrix &JJ, 
                                    Matrix &JJinv )
{
...
void  
ShellMITC4GNLThermal::shape2d( double ss, double tt, 
		           const double x[2][4], 
		           double shp[3][4], 
		           double &xsj            )
...
double
ShellMITC4GNLThermal::shapefn2d( double ss, double tt ,int i)
...
Matrix  
ShellMITC4GNLThermal::transpose( int dim1, 
                                       int dim2, 
		                       const Matrix &M ) 
{
  int i ;
...
int  ShellMITC4GNLThermal::sendSelf (int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING ShellMITC4GNLThermal::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "WARNING ShellMITC4GNLThermal::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }
...
int  ShellMITC4GNLThermal::recvSelf (int commitTag, 
		       Channel &theChannel, 
		       FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ShellMITC4GNLThermal::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (materialPointers[0] == 0) {
    for (i = 0; i < 4; i++) {
      ...
      if (materialPointers[i] == 0) {
	opserr << "ShellMITC4GNLThermal::recvSelf() - Broker could not create NDMaterial of class type" << matClassTag << endln;;
	return -1;
      }
      ...
      if (res < 0) {
	opserr << "ShellMITC4GNLThermal::recvSelf() - material " << i << "failed to recv itself\n";
	return res;
      }
    }
  }
...
int
ShellMITC4GNLThermal::displaySelf(Renderer &theViewer, int displayMode, float fact)
{
