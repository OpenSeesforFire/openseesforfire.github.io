...
OPS_NewShellMITC4Thermal(void)
{
  if (numShellMITC4Thermal == 0) {
    opserr << "Using ShellMITC4Thermal - ShellMITC4 was developed by: Leopoldo Tesser, Diego A. Talledo, Veronique Le Corvec\n";
	opserr << "                        - Modified for Thermo-mechanical analysis by Jian Jiang, Liming Jiang\n";
...
Matrix**  ShellMITC4Thermal::GammaB1pointer = 0 ;
Matrix**  ShellMITC4Thermal::GammaD1pointer = 0 ;
Matrix**  ShellMITC4Thermal::GammaA2pointer = 0 ;
Matrix**  ShellMITC4Thermal::GammaC2pointer = 0 ; 
Matrix**  ShellMITC4Thermal::Bhat           = 0 ;
...
/*
const double  ShellMITC4Thermal::sg[] = { -one_over_root3,  
				one_over_root3, 
				one_over_root3, 
	                       -one_over_root3 } ;

const double  ShellMITC4Thermal::tg[] = { -one_over_root3, 
                               -one_over_root3, 
                                one_over_root3,  
                                one_over_root3 } ;

const double  ShellMITC4Thermal::wg[] = { 1.0, 1.0, 1.0, 1.0 } ;
*/
...
connectedExternalNodes(4), load(0), Ki(0), isNL(false)
{ 
  for (int i = 0 ;  i < 4; i++ ) 
...
connectedExternalNodes(4), load(0), Ki(0),isNL(true)
{
  int i;
...
const Matrix&  ShellMITC4Thermal::getTangentStiff( ) 
{
 ...
 #ifdef _SDEBUG
opserr<< "ShellMITC4Thermal: "<<this->getTag()<< "getTangent stiffness: "<<endln;
//<<stiff<<endln;
#endif
...
const Matrix&  ShellMITC4Thermal::getInitialStiff( ) 
{
  ...
  #ifdef _SDEBUG
opserr<< "ShellMITC4Thermal: "<<this->getTag()<< "Initial Tangent stiffness: "<<endln;
//<<stiff<<endln;
#endif
  return stiff ;
}
...
#ifdef _DEBUG
  if(this->getTag()==2||this->getTag()==10){
  opserr<< "ShellMITC4Thermal: "<<this->getTag()<< " ResidThermal(before thermal): "
	    << resid<<endln;
  }
#endif
...
#ifdef _SDEBUG
 opserr<< "ShellMITC4Thermal: "<<this->getTag()<< " ResidThermal(after thermal): "<<endln
	 <<resid<<endln;
#endif
...
//
...
#ifdef _DEBUG
	  opserr<<"ShellMITC4:"<<this->getTag()<< " GETDISP "<<ul<<endln;
#endif
      //compute the strain
      //strain += (BJ*ul) ; 
  ...
  // Adding geoemtric stiffness components
   //add kij to stiff
    if (isNL) {
...
#ifdef _SDEBUG
  if(this->getTag()==1){
opserr<< "ShellMITC4Thermal: "<<this->getTag()<< "Update-> Tangent stiffness: "<<endln
<<stiff<<endln;
  }
#endif
...
#ifdef _SDEBUG
	if(this->getTag()==1){
		opserr<< "SHELLMITC4::STRESS  "<<stress<<endln;
	}
#endif
...
void   
ShellMITC4Thermal::computeGamma( const double xl[2][4], const Matrix &J )
{
...
const Matrix&  
ShellMITC4Thermal::computeBbarShear( int node, double L1, double L2,
			      const Matrix &Jinv ) 
{
...
const Matrix&  
ShellMITC4Thermal::computeBshear( int node, const double shp[3][4] )
{
...
void   
ShellMITC4Thermal::computeJacobian( double L1, double L2,
				    const double x[2][4], 
                                    Matrix &JJ, 
                                    Matrix &JJinv )
{
