...
#ifndef DispBeamColumn2dThermalAI_h
#define DispBeamColumn2dThermalAI_h
...
class DispBeamColumn2dThermalAI : public Element
{
public:
    DispBeamColumn2dThermalAI(int tag, int nd1, int nd2,
        int numSections, SectionForceDeformation** s,
        BeamIntegration& bi, CrdTransf& coordTransf, const char* path,
        double rho = 0.0);
    DispBeamColumn2dThermalAI();
    ~DispBeamColumn2dThermalAI();

    const char* getClassType(void) const { return "DispBeamColumn2dThermalAI"; };
...
private:
    ...
    double disp;
    double temp;
    double getPredictedStiffness(double Temp, double disp);
    double getPredictedForce(double Temp, double disp);
};
