...
// $Source: /usr/local/cvs/OpenSees/SRC/element/dispBeamColumn/DispBeamColumn3dThermalAI2.cpp,v $

// Written: MHS
// Created: Feb 2001
//
// Description: This file contains the class definition for DispBeamColumn3dThermalAI2.

#include <DispBeamColumn3dThermalAI2.h>
#include <Node.h>
...
#include <Python.h>
Matrix DispBeamColumn3dThermalAI2::K(12, 12);
Vector DispBeamColumn3dThermalAI2::P(12);
double DispBeamColumn3dThermalAI2::RP;
double DispBeamColumn3dThermalAI2::workArea[200];

void* OPS_DispBeamColumn3dThermalAI2()
{
    ...
    Element* theEle = new DispBeamColumn3dThermalAI2(iData[0], iData[1], iData[2], secTags.Size(), sections,
        *bi, *theTransf, mass);
    delete[] sections;
    return theEle;
}

DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2(int tag, int nd1, int nd2,
    int numSec, SectionForceDeformation** s,
    BeamIntegration& bi,
    CrdTransf& coordTransf, double r)
    :Element(tag, ELE_TAG_DispBeamColumn3dThermalAI2),
    eleTag(tag),
    ...
    Q(12), q(6), rho(r), parameterID(0)
{
    ...
    if (theSections == 0) {
        opserr << "DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2 - failed to allocate section model pointer\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        // Check allocation
        if (theSections[i] == 0) {
            opserr << "DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2 -- failed to get a copy of section model\n";
            exit(-1);
        }
    }
    ...
    if (beamInt == 0) {
        opserr << "DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2 - failed to copy beam integration\n";
        exit(-1);
    }
    ...
    if (crdTransf == 0) {
        opserr << "DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2 - failed to copy coordinate transformation\n";
        exit(-1);
    }
...
DispBeamColumn3dThermalAI2::DispBeamColumn3dThermalAI2()
    :Element(0, ELE_TAG_DispBeamColumn3dThermalAI2),
...
double
DispBeamColumn3dThermalAI2::getPredictedStiffness(int eleTag, double Temperature, double deflection)
{
    ...
    //opserr << "KeleTag" << eleTag << endln;
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "AI Failed to load the module1." << endln;
    }
...
double
DispBeamColumn3dThermalAI2::getPredictedForce(int eleTag, double Temperature, double deflection) //
{ 
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
    //opserr << "PeleTag" << eleTag << endln;
    PyRun_SimpleString("import sys");
    //PyRun_SimpleString("sys.path.append('path')");
    PyRun_SimpleString("sys.path.append(r'D:\\hybrid simulation\\DL')");
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "AI Failed to load the module2." << endln;
    }
...
double
DispBeamColumn3dThermalAI2::store(int eleTag, double Temperature, double deflection, double RP)
{
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "AI Failed to load the module3." << endln;
    }

    pFunc = PyObject_GetAttrString(pModule, "update");

    if (pFunc == NULL) {
        opserr << "Failed to get function 'update'." << endln;
    }
    ...
    PyTuple_SetItem(pArgs, 1, PyFloat_FromDouble(Temperature));
    PyTuple_SetItem(pArgs, 2, PyFloat_FromDouble(deflection));
    PyTuple_SetItem(pArgs, 3, PyFloat_FromDouble(RP));
    pValue = PyObject_CallObject(pFunc, pArgs);
    float M = PyFloat_AsDouble(pValue);

    return M;
}

int
DispBeamColumn3dThermalAI2::getNumExternalNodes() const
{
    return 2;
}
...
DispBeamColumn3dThermalAI2::getExternalNodes()
{
    return connectedExternalNodes;
}

Node**
DispBeamColumn3dThermalAI2::getNodePtrs()
{
...
int
DispBeamColumn3dThermalAI2::getNumDOF()
{
    return 12;
}

void
DispBeamColumn3dThermalAI2::setDomain(Domain* theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
        //opserr << "FATAL ERROR DispBeamColumn3dThermalAI2 (tag: %d), node not found in domain",
        //	this->getTag());
    ...
    if (dofNd1 != 6 || dofNd2 != 6) {
        //opserr << "FATAL ERROR DispBeamColumn3dThermalAI2 (tag: %d), has differing number of DOFs at its nodes",
        //	this->getTag());
...
int
DispBeamColumn3dThermalAI2::commitState()
{
    int retVal = 0;
    // Call AI model to store current trial
    // opserr << deflection << endln;
    double AI = store(eleTag, Temperature, deflection, RP);
    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
        opserr << "DispBeamColumn3dThermalAI2::commitState () - failed in base class";
    }
...
int
DispBeamColumn3dThermalAI2::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumn3dThermalAI2::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumn3dThermalAI2::update(void)
{
    int err = 0;
    ...
    deflection = v[0];
    Temperature = ((FiberSection3dThermal*)theSections[0])->getTemperature();
    ...
    if (err != 0) {
        opserr << "DispBeamColumn3dThermalAI2::update() - failed setTrialSectionDeformations()\n";
        return err;
    }
    return 0;
}
...
const Matrix&
DispBeamColumn3dThermalAI2::getTangentStiff()
{
    ...
    K(2, 2) = getPredictedStiffness(eleTag, Temperature, deflection);
    K(2, 8) = -K(2, 2);//getPredictedStiffness(temp, disp);
    K(8, 2) = -K(2, 2);//getPredictedStiffness(temp, disp);
    K(8, 8) = K(2, 2);//getPredictedStiffness(temp, disp) * (-1);
    //opserr << "K: " << K << endln;
#ifdef _bDEBUG
    if (this->getTag() == 601 || this->getTag() == 602) {
...
const Matrix&
DispBeamColumn3dThermalAI2::getInitialBasicStiff()
{
    static Matrix kb(6, 6);
...
const Matrix&
DispBeamColumn3dThermalAI2::getInitialStiff()
{
...
const Matrix&
DispBeamColumn3dThermalAI2::getMass()
{
    K.Zero();
...
void
DispBeamColumn3dThermalAI2::zeroLoad(void)
{
    Q.Zero();
...
int
DispBeamColumn3dThermalAI2::addLoad(ElementalLoad* theLoad, double loadFactor)
{
    int type;
    ...
    //----------------------------------------------
        ...
        opserr << "DispBeamColumn3dThermalAI2::addLoad() -- load type unknown for element with tag: " <<
            this->getTag() << endln;
        return -1;
    }
...
int
DispBeamColumn3dThermalAI2::addInertiaLoadToUnbalance(const Vector& accel)
{
    ...
    if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
        opserr << "DispBeamColumn3dThermalAI2::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
        return -1;
    }
...
const Vector&
DispBeamColumn3dThermalAI2::getResistingForce()
{
    double L = crdTransf->getInitialLength();
    ...
    //opserr<<"Beam "<<q<<endln;
    P(2) = getPredictedForce(eleTag, Temperature, deflection);
    RP = P(2);
    //opserr << "temp: " << Temperature << endln;
    //opserr << "disp: " << deflection << endln;
    //opserr << "Predicted F2: " << P(2) << endln;
    P(8) = P(2) * (-1);
    //opserr << "P: " << P << endln;
...
const Vector&
DispBeamColumn3dThermalAI2::getResistingForceIncInertia()
{
...
int
DispBeamColumn3dThermalAI2::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to send ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to send double data\n";
            return -1;
        }
    }
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to send crdTranf\n";
        return -1;
    }

    // send the beam integration
    if (beamInt->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to send beamInt\n";
        return -1;
    }
    ...
    for (j = 0; j < numSections; j++) {
        if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
            opserr << "DispBeamColumn3dThermalAI2::sendSelf() - section " << j << "failed to send itself\n";
            return -1;
        }
    }
...
int
DispBeamColumn3dThermalAI2::recvSelf(int commitTag, Channel& theChannel,
    FEM_ObjectBroker& theBroker)
{
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn3dThermalAI2::recvSelf() - failed to recv ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to recv double data\n";
            return -1;
        }
        alphaM = dData(0);
        betaK = dData(1);
    ...
    if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
        ...
        if (crdTransf == 0) {
            opserr << "DispBeamColumn3dThermalAI2::recvSelf() - " <<
    ...
    if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to recv crdTranf\n";
        return -3;
    }
    ...
    if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
        ...
        if (beamInt == 0) {
            opserr << "DispBeamColumn3dThermalAI2::recvSelf() - failed to obtain the beam integration object with classTag" <<
    ...
    {
        opserr << "DispBeamColumn3dThermalAI2::sendSelf() - failed to recv beam integration\n";
        return -3;
    }
    ...
    if (numSections != idData(3)) {
        ...
        if (theSections == 0) {
            opserr << "DispBeamColumn3dThermalAI2::recvSelf() - out of memory creating sections array of size" <<
                idData(3) << endln;
            exit(-1);
        }
        ...
        for (i = 0; i < numSections; i++) {
            ...
            if (theSections[i] == 0) {
                opserr << "DispBeamColumn3dThermalAI2::recvSelf() - Broker could not create Section of class type" <<
                    sectClassTag << endln;
                exit(-1);
            }
            theSections[i]->setDbTag(sectDbTag);
            if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "DispBeamColumn3dThermalAI2::recvSelf() - section " <<
...
void
DispBeamColumn3dThermalAI2::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nDispBeamColumn3dThermalAI2, element id:  " << this->getTag() << endln;
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{";
        s << "\"name\": " << this->getTag() << ", ";
        s << "\"type\": \"DispBeamColumn3dThermalAI2\", ";
...
int
DispBeamColumn3dThermalAI2::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** modes, int numModes)
{
    static Vector v1(3);
    static Vector v2(3);
...
Response*
DispBeamColumn3dThermalAI2::setResponse(const char** argv, int argc, OPS_Stream& output)
{

    Response* theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType", "DispBeamColumn3dThermalAI2");
...
int
DispBeamColumn3dThermalAI2::getResponse(int responseID, Information& eleInfo)
{
    double N, V, M1, M2, T;
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
int
DispBeamColumn3dThermalAI2::setParameter(const char** argv, int argc, Parameter& param)
{
    if (argc < 1)
        return -1;
...
int
DispBeamColumn3dThermalAI2::updateParameter(int parameterID, Information& info)
{
    if (parameterID == 1) {
        rho = info.theDouble;
        return 0;
...
int
DispBeamColumn3dThermalAI2::activateParameter(int passedParameterID)
{
...
const Matrix&
DispBeamColumn3dThermalAI2::getKiSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Matrix&
DispBeamColumn3dThermalAI2::getMassSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Vector&
DispBeamColumn3dThermalAI2::getResistingForceSensitivity(int gradNumber)
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
// NEW METHOD
int
DispBeamColumn3dThermalAI2::commitSensitivity(int gradNumber, int numGrads)
{
