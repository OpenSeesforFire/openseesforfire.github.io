...
// $Source: /usr/local/cvs/OpenSees/SRC/element/dispBeamColumn/DispBeamColumn2dThermalNUTNUT.cpp,v $
...
// Modified: Liming Jiang[Univeristy of Edinburgh]



#include <DispBeamColumn2dThermalNUT.h>
#include <Node.h>
...
Matrix DispBeamColumn2dThermalNUT::K(6,6);
Vector DispBeamColumn2dThermalNUT::P(6);
double DispBeamColumn2dThermalNUT::workArea[100];

DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT(int tag, int nd1, int nd2,
				   int numSec, SectionForceDeformation **s,
				   BeamIntegration& bi,
				   CrdTransf &coordTransf, double r)
:Element (tag, ELE_TAG_DispBeamColumn2dThermalNUT), 
  ...
  Q(6), q(3), rho(r), parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT - failed to allocate section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT -- failed to get a copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT - failed to copy beam integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT - failed to copy coordinate transformation\n";
    exit(-1);
  }
...
DispBeamColumn2dThermalNUT::DispBeamColumn2dThermalNUT()
:Element (0, ELE_TAG_DispBeamColumn2dThermalNUT),
...
int
DispBeamColumn2dThermalNUT::getNumExternalNodes() const
{
    return 2;
}

const ID&
DispBeamColumn2dThermalNUT::getExternalNodes()
{
    return connectedExternalNodes;
}

Node **
DispBeamColumn2dThermalNUT::getNodePtrs()
{
    return theNodes;
}

int
DispBeamColumn2dThermalNUT::getNumDOF()
{
    return 6;
}

void
DispBeamColumn2dThermalNUT::setDomain(Domain *theDomain)
{
	...
	//opserr << "FATAL ERROR DispBeamColumn2dThermalNUT (tag: %d), node not found in domain",
	//	this->getTag());
	...
	//opserr << "FATAL ERROR DispBeamColumn2dThermalNUT (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
int
DispBeamColumn2dThermalNUT::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "DispBeamColumn2dThermalNUT::commitState () - failed in base class";
    }    
...
int
DispBeamColumn2dThermalNUT::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalNUT::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalNUT::update(void)
{
  ...
  #ifdef _DEBUG
  opserr<<"BeamNUT: v  "<<v<<endln<<"BeamNUT: Average epsT "<<AverageThermalElong<<endln;
  #endif
  for (int i = 0; i < numSections; i++) {
  ...
  if (err != 0) {
    opserr << "DispBeamColumn2dThermalNUT::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
const Matrix&
DispBeamColumn2dThermalNUT::getTangentStiff()
{
...
const Matrix&
DispBeamColumn2dThermalNUT::getInitialBasicStiff()
{
  static Matrix kb(3,3);
...
const Matrix&
DispBeamColumn2dThermalNUT::getInitialStiff()
{
...
const Matrix&
DispBeamColumn2dThermalNUT::getMass()
{
  K.Zero();
...
void
DispBeamColumn2dThermalNUT::zeroLoad(void)
{
  Q.Zero();
...
int 
DispBeamColumn2dThermalNUT::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    for (int i = 0; i < numSections; i++) {
	  ...
	  SectionThermalElong[i]= ((FiberSection2dThermal*)theSections[i])->getThermalElong();
    ...
    q0[2] -= 0;
	   ...
	   opserr << "DispBeamColumn2dThermalNUT::addLoad(double) -- load type " << theLoad->getClassType() 
...
int 
DispBeamColumn2dThermalNUT::addLoad(ElementalLoad *theLoad, const Vector &factors)
{
  int type;
 ...
 //JZ 07/10 /////////////////////////////////////////////////////////////start
  ...
  //PK
    ...
    q0[2] -= 0;
	  ...
	  opserr << "DispBeamColumn2dThermalNUT::addLoad(Vector) -- load type " << theLoad->getClassType() 
		  << "unknown for element with tag: " << this->getTag() << "\n"; 
    return -1;
  }
...
int 
DispBeamColumn2dThermalNUT::addInertiaLoadToUnbalance(const Vector &accel)
{
    ...
    if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
      opserr << "DispBeamColumn2dThermalNUT::addInertiaLoadToUnbalance matrix and vector sizes are incompatable\n";
      return -1;
    }
...
const Vector&
DispBeamColumn2dThermalNUT::getResistingForce()
{
  double L = crdTransf->getInitialLength();
  double oneOverL = 1.0/L;
...
const Vector&
DispBeamColumn2dThermalNUT::getResistingForceIncInertia()
{
...
int
DispBeamColumn2dThermalNUT::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
    opserr << "DispBeamColumn2dThermalNUT::sendSelf() - failed to send ID data\n";
     return -1;
  }    
  ...
  if (idData(6) == 1) {
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
      opserr << "DispBeamColumn2dThermalNUT::sendSelf() - failed to send double data\n";
      return -1;
    }    
  }
  ...
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
     opserr << "DispBeamColumn2dThermalNUT::sendSelf() - failed to send crdTranf\n";
     return -1;
  }      
  ...
  for (j = 0; j<numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "DispBeamColumn2dThermalNUT::sendSelf() - section " << 
...
int
DispBeamColumn2dThermalNUT::recvSelf(int commitTag, Channel &theChannel,
			   FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
    opserr << "DispBeamColumn2dThermalNUT::recvSelf() - failed to recv ID data\n";
    return -1;
  }    
  ...
  if (idData(6) == 1) {
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
      opserr << "DispBeamColumn2dThermalNUT::sendSelf() - failed to recv double data\n";
      return -1;
    }    
    alphaM = dData(0);
    betaK = dData(1);
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "DispBeamColumn2dThermalNUT::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
  ...
  if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "DispBeamColumn2dThermalNUT::sendSelf() - failed to recv crdTranf\n";
    return -3;
  }      
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
opserr << "DispBeamColumn2dThermalNUT::recvSelf() - out of memory creating sections array of size " <<
  idData(3) << endln;
      return -1;
    }    
    ...
    for (i=0; i<numSections; i++) {
      ...
      if (theSections[i] == 0) {
	opserr << "DispBeamColumn2dThermalNUT::recvSelf() - Broker could not create Section of class type " <<
	  sectClassTag << endln;
	exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	opserr << "DispBeamColumn2dThermalNUT::recvSelf() - section " << i << " failed to recv itself\n";
	return -1;
      }     
    }
...
void
DispBeamColumn2dThermalNUT::Print(OPS_Stream &s, int flag)
{
  s << "\nDispBeamColumn2dThermalNUT, element id:  " << this->getTag() << endln;
...
int
DispBeamColumn2dThermalNUT::displaySelf(Renderer &theViewer, int displayMode, float fact)
{
...
Response*
DispBeamColumn2dThermalNUT::setResponse(const char **argv, int argc,
			      OPS_Stream &output)
{
  Response *theResponse = 0;

  output.tag("ElementOutput");
  output.attr("eleType","DispBeamColumn2dThermalNUT");
...
int 
DispBeamColumn2dThermalNUT::getResponse(int responseID, Information &eleInfo)
{
  double V;
  double L = crdTransf->getInitialLength();
...
int
DispBeamColumn2dThermalNUT::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
DispBeamColumn2dThermalNUT::updateParameter (int parameterID, Information &info)
{
  if (parameterID == 1) {
    rho = info.theDouble;
    return 0;
...
int
DispBeamColumn2dThermalNUT::activateParameter(int passedParameterID)
{
...
const Matrix &
DispBeamColumn2dThermalNUT::getKiSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Matrix &
DispBeamColumn2dThermalNUT::getMassSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Vector &
DispBeamColumn2dThermalNUT::getResistingForceSensitivity(int gradNumber)
{
  double L = crdTransf->getInitialLength();
  double oneOverL = 1.0/L;
...
// NEW METHOD
int
DispBeamColumn2dThermalNUT::commitSensitivity(int gradNumber, int numGrads)
{
