...
#include <DispBeamColumn2dThermalAI2.h>
#include <Python.h>

Matrix DispBeamColumn2dThermalAI2::K(6, 6);
Vector DispBeamColumn2dThermalAI2::P(6);
double DispBeamColumn2dThermalAI2::workArea[100];

void* OPS_DispBeamColumn2dThermalAI2()
{
    if (OPS_GetNumRemainingInputArgs() < 6) {
        opserr << "insufficient arguments:eleTag,iNode,jNode,transfTag,integrationTag, <-mass mass> <-cmass>, path\n";
        return 0;
    }
    ...
    //const char* path = OPS_GetString();
    if (OPS_GetNumRemainingInputArgs() < 0) {
        opserr << "WARNING invalid function path" << endln;
        return 0;
    }
    else {
        const char* path = OPS_GetString();
    ...
    Element* theEle = new DispBeamColumn2dThermalAI2(iData[0], iData[1], iData[2], secTags.Size(), sections,
        *bi, *theTransf, path, mass);
    //opserr << "path2:" << path <<endln;
    delete[] sections;
    return theEle;
}

DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2(int tag, int nd1, int nd2,
    int numSec, SectionForceDeformation** s,
    BeamIntegration& bi,
    CrdTransf& coordTransf, const char* path, double r)
    :Element(tag, ELE_TAG_DispBeamColumn2dThermalAI2),
    eleTag(tag),
    ...
    Q(6), q(3), rho(r), parameterID(0)
{
    ...
    if (theSections == 0) {
        opserr << "DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2 - failed to allocate section model pointer\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        // Check allocation
        if (theSections[i] == 0) {
            opserr << "DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2 -- failed to get a copy of section model\n";
            exit(-1);
        }
    }
    ...
    if (beamInt == 0) {
        opserr << "DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2 - failed to copy beam integration\n";
        exit(-1);
    }
    ...
    if (crdTransf == 0) {
        opserr << "DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2 - failed to copy coordinate transformation\n";
        exit(-1);
    }
...
DispBeamColumn2dThermalAI2::DispBeamColumn2dThermalAI2()
    :Element(0, ELE_TAG_DispBeamColumn2dThermalAI2),
...
double
DispBeamColumn2dThermalAI2::getPredictedStiffness(int eleTag, double Temperature, double deflection)
{
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
    PyRun_SimpleString("import sys");
    // PyRun_SimpleString("sys.path.append('path')");
    PyRun_SimpleString("sys.path.append(r'D:\\hybrid simulation\\DL')");
    
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "Failed to load the module1." << endln;
    }
    pFunc = PyObject_GetAttrString(pModule, "predicted_K");
    if (pFunc == NULL) {
        opserr << "Failed to get function 'predicted_k'." << endln;
    }
    pArgs = PyTuple_New(3);
    PyTuple_SetItem(pArgs, 0, PyFloat_FromDouble(eleTag));
    PyTuple_SetItem(pArgs, 1, PyFloat_FromDouble(Temperature));
    PyTuple_SetItem(pArgs, 2, PyFloat_FromDouble(deflection));

    pValue = PyObject_CallObject(pFunc, pArgs);
    float K = PyFloat_AsDouble(pValue);
    return K;
}              

double
DispBeamColumn2dThermalAI2::getPredictedForce(int eleTag, double Temperature, double deflection)
{
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
    ...
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "Failed to load the module2." << endln;
    }

    pFunc = PyObject_GetAttrString(pModule, "predicted_F");

    if (pFunc == NULL) {
        opserr << "Failed to get function 'predicted_F'." << endln;
    }
    ...
    pValue = PyObject_CallObject(pFunc, pArgs);
    float F = PyFloat_AsDouble(pValue);

    return F;
}                 

int
DispBeamColumn2dThermalAI2::getNumExternalNodes() const
{
    return 2;
}

const ID&
DispBeamColumn2dThermalAI2::getExternalNodes()
{
    return connectedExternalNodes;
}

Node**
DispBeamColumn2dThermalAI2::getNodePtrs()
{
    return theNodes;
}

int
DispBeamColumn2dThermalAI2::getNumDOF()
{
    return 6;
}

void
DispBeamColumn2dThermalAI2::setDomain(Domain* theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
        opserr << "FATAL ERROR DispBeamColumn2dThermalAI2 (tag: %d), node not found in domain";
        //	this->getTag());
    ...
    if (dofNd1 != 3 || dofNd2 != 3) {
        opserr << "FATAL ERROR DispBeamColumn2dThermalAI2 (tag: %d), has differing number of DOFs at its nodes";
        //	this->getTag());
...
int
DispBeamColumn2dThermalAI2::commitState()
{
    int retVal = 0;
    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
        opserr << "DispBeamColumn2dThermalAI2::commitState () - failed in base class";
    }
...
int
DispBeamColumn2dThermalAI2::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalAI2::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalAI2::update(void)
{
    int err = 0;
    ...
    disp = v[0]; 
    temp = ((FiberSection2dThermal*)theSections[0])->getTemperature();
    //opserr<< "Basic Deformation: "<< disp <<endln;
...
//opserr << "BeamNUT_" << this->getTag() << " ,  v  " << v << endln;
    ...
    if (err != 0) {
        opserr << "DispBeamColumn2dThermalAI2::update() - failed setTrialSectionDeformations()\n";
        return err;
    }
...
const Matrix&
DispBeamColumn2dThermalAI2::getTangentStiff()
{
    ...
    //opserr << "old K:" << K << endln;
    //if (temp > 0) {
    //    K(1, 1) = getPredictedStiffness(temp, disp) * (-1); //QJ
    //    K(1, 4) = getPredictedStiffness(temp, disp); //QJ
    //    K(4, 1) = getPredictedStiffness(temp, disp); //QJ
    //    K(4, 4) = getPredictedStiffness(temp, disp) * (-1); //QJ
    //    //opserr << "Predicted F:" << K(0, 1) << endln;
    //}
    //else {

    //    K = K;
    //}
    K(1, 1) = getPredictedStiffness(eleTag, temp, disp);
    K(1, 4) = -K(1, 1);//getPredictedStiffness(temp, disp);
    K(4, 1) = -K(1, 1);//getPredictedStiffness(temp, disp);
    K(4, 4) = K(1, 1);//getPredictedStiffness(temp, disp) * (-1);
    //K(2, 2) = getPredictedStiffness(temp, disp) * (-1);
    //K(2, 5) = getPredictedStiffness(temp, disp);
    //K(5, 2) = getPredictedStiffness(temp, disp);
    //K(5, 5) = getPredictedStiffness(temp, disp) * (-1);
    //opserr << "New K: " << K << endln;
    return K;
}

const Matrix&
DispBeamColumn2dThermalAI2::getInitialBasicStiff()
{
    static Matrix kb(3, 3);
...
const Matrix&
DispBeamColumn2dThermalAI2::getInitialStiff()
{
...
const Matrix&
DispBeamColumn2dThermalAI2::getMass()
{
    K.Zero();
...
void
DispBeamColumn2dThermalAI2::zeroLoad(void)
{
    Q.Zero();
...
int
DispBeamColumn2dThermalAI2::addLoad(ElementalLoad* theLoad, double loadFactor)
{
    int type;
    const Vector& data = theLoad->getData(type, loadFactor);
    double L = crdTransf->getInitialLength();
    // temp = data(0);
    // opserr << data << endln;  //check temp input
    if (type == LOAD_TAG_Beam2dUniformLoad) {
        ...
        for (int i = 0; i < 9; i++) {
            if (data0(2 * i + 1) - data1(2 * i + 1) > 1e-8 || data0(2 * i + 1) - data1(2 * i + 1) < -1e-8) {
                opserr << "Warning:The NodalThermalAction in dispBeamColumn2dThermalAINUT " << this->getTag()
...
#ifdef _BDEBUG
            opserr << "DispBeamColumn2dThermalAI2::addLoad at the ele " << this->getTag() << " section " << i << " --theIntCrds" << theIntCrds << endln;
#endif
    ...
    //----------------------------------------
        ...
        opserr << "DispBeamColumn2dThermalAI2::addLoad(double) -- load type " << theLoad->getClassType()
...
int
DispBeamColumn2dThermalAI2::addLoad(ElementalLoad* theLoad, const Vector& factors)
{
    int type;
   ...
   //JZ 07/10 /////////////////////////////////////////////////////////////start
    if (type == LOAD_TAG_Beam2dThermalAction) {
        ...
        opserr << "DispBeamColumn2dThermalAI2::addLoad(Vector) -- load type " << theLoad->getClassType()
            << "unknown for element with tag: " << this->getTag() << "\n";
        return -1;
    }
...
int
DispBeamColumn2dThermalAI2::addInertiaLoadToUnbalance(const Vector& accel)
{
    ...
    if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
        opserr << "DispBeamColumn2dThermalAI2::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
        return -1;
    }
...
const Vector&
DispBeamColumn2dThermalAI2::getResistingForce()
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
    ...
    //opserr << "old Force: " << P(1) << endln;
    //if (temp > 0) {
    //P(1) = getPredictedForce(temp, disp);
    //P(4) = getPredictedForce(temp, disp) * (-1);

    ////opserr << "Predicted F2:" << P(4) << endln;
    //}
    //else {
    //    P(1) = P(1);
    //    P(4) = P(4);
    //}
    P(1) = getPredictedForce(eleTag, temp, disp);
    opserr << "temp: " << temp << endln;
    opserr << "disp: " << disp << endln;
    opserr << "Predicted F1: " << P(1) << endln;
    P(4) = P(1) * (-1);
    //opserr << "new Force: " << P(1) << endln;
    // Subtract other external nodal loads ... P_res = P_int - P_ext
    //P.addVector(1.0, Q, -1.0);
  // opserr <<" Ele: "<< this->getTag()<< ", P "<<P<<endln;
    P(0) -= Q(0);  //a-=b means a=a-b
    P(1) -= Q(1);
    P(2) -= Q(2);
    P(3) -= Q(3);
    P(4) -= Q(4);
...
const Vector&
DispBeamColumn2dThermalAI2::getResistingForceIncInertia()
{
...
int
DispBeamColumn2dThermalAI2::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn2dThermalAI2::sendSelf() - failed to send ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn2dThermalAI2::sendSelf() - failed to send double data\n";
            return -1;
        }
    }
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn2dThermalAI2::sendSelf() - failed to send crdTranf\n";
        return -1;
    }
    ...
    for (j = 0; j < numSections; j++) {
        if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
            opserr << "DispBeamColumn2dThermalAI2::sendSelf() - section " <<
...
int
DispBeamColumn2dThermalAI2::recvSelf(int commitTag, Channel& theChannel,
    FEM_ObjectBroker& theBroker)
{
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn2dThermalAI2::recvSelf() - failed to recv ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn2dThermalAI2::sendSelf() - failed to recv double data\n";
            return -1;
        }
        alphaM = dData(0);
        betaK = dData(1);
    ...
    if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
        ...
        if (crdTransf == 0) {
            opserr << "DispBeamColumn2dThermalAI2::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
    ...
    if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn2dThermalAI2::sendSelf() - failed to recv crdTranf\n";
        return -3;
    }
    ...
    if (numSections != idData(3)) {
        ...
        if (theSections == 0) {
            opserr << "DispBeamColumn2dThermalAI2::recvSelf() - out of memory creating sections array of size " <<
                idData(3) << endln;
            return -1;
        }
        ...
        for (i = 0; i < numSections; i++) {
            ...
            if (theSections[i] == 0) {
                opserr << "DispBeamColumn2dThermalAI2::recvSelf() - Broker could not create Section of class type " <<
                    sectClassTag << endln;
                exit(-1);
            }
            theSections[i]->setDbTag(sectDbTag);
            if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "DispBeamColumn2dThermalAI2::recvSelf() - section " << i << " failed to recv itself\n";
                return -1;
            }
        }
...
void
DispBeamColumn2dThermalAI2::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nDispBeamColumn2dThermalAI2, element id:  " << this->getTag() << endln;
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{";
        s << "\"name\": " << this->getTag() << ", ";
        s << "\"type\": \"DispBeamColumn2dThermalAI2\", ";
...
int
DispBeamColumn2dThermalAI2::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** displayModes, int numModes)
{
    static Vector v1(3);
    static Vector v2(3);
...
Response*
DispBeamColumn2dThermalAI2::setResponse(const char** argv, int argc,
    OPS_Stream& output)
{
    Response* theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType", "DispBeamColumn2dThermalAI2");
...
int
DispBeamColumn2dThermalAI2::getResponse(int responseID, Information& eleInfo)
{
    double V;
    double L = crdTransf->getInitialLength();
...
int
DispBeamColumn2dThermalAI2::setParameter(const char** argv, int argc, Parameter& param)
{
    if (argc < 1)
        return -1;
...
int
DispBeamColumn2dThermalAI2::updateParameter(int parameterID, Information& info)
{
    if (parameterID == 1) {
        rho = info.theDouble;
        return 0;
...
int
DispBeamColumn2dThermalAI2::activateParameter(int passedParameterID)
{
...
const Matrix&
DispBeamColumn2dThermalAI2::getKiSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Matrix&
DispBeamColumn2dThermalAI2::getMassSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Vector&
DispBeamColumn2dThermalAI2::getResistingForceSensitivity(int gradNumber)
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
// NEW METHOD
int
DispBeamColumn2dThermalAI2::commitSensitivity(int gradNumber, int numGrads)
{
