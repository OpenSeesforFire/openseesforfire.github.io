...
//J.Jiang add to consider thermal load
   ...
   /// This code block is added by LJ and copied from DispBeamColumn2d(Modified edition) for 'FireLoadPattern'--08-May-2012--//[END]
	...
	//q.Zero();
      ...
      dataMixV=data;
#ifdef _DEBUG
	 opserr<<"dataMix in Element"<<dataMixV<<endln;
#endif
	// Loop over the integration points
	for (int i = 0; i < numSections; i++) {
  ...
  const Vector &data1 = theNodalThermal0->getData(type);
  Vector Loc(10);
  Vector NodalT0(15);
  Vector NodalT1(15);
...
DispBeamColumn3dThermal::getResistingForce()
{
  ...
  if (counterTemperature == 0)//J.Jiang add to consider thermal load at first step
	{
	for ( int i=0; i<5; i++)
    q(i) +=residThermal[i];
	}
