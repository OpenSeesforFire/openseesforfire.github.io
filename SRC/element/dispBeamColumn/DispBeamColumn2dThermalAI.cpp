...
#include <DispBeamColumn2dThermalAI.h>
#include <Python.h>

Matrix DispBeamColumn2dThermalAI::K(6, 6);
Vector DispBeamColumn2dThermalAI::P(6);
double DispBeamColumn2dThermalAI::workArea[100];
...
void* OPS_DispBeamColumn2dThermalAI()
{
    ...
    Element* theEle = new DispBeamColumn2dThermalAI(iData[0], iData[1], iData[2], secTags.Size(), sections,
...
DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI(int tag, int nd1, int nd2,
    int numSec, SectionForceDeformation** s,
    BeamIntegration& bi,
    CrdTransf& coordTransf, const char* path, double r)
    :Element(tag, ELE_TAG_DispBeamColumn2dThermalAI),
    ...
    Q(6), q(3), rho(r), parameterID(0)
{
    ...
    if (theSections == 0) {
        opserr << "DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI - failed to allocate section model pointer\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        // Check allocation
        if (theSections[i] == 0) {
            opserr << "DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI -- failed to get a copy of section model\n";
            exit(-1);
        }
    }
    ...
    if (beamInt == 0) {
        opserr << "DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI - failed to copy beam integration\n";
        exit(-1);
    }
    ...
    if (crdTransf == 0) {
        opserr << "DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI - failed to copy coordinate transformation\n";
        exit(-1);
    }
...
DispBeamColumn2dThermalAI::DispBeamColumn2dThermalAI()
    :Element(0, ELE_TAG_DispBeamColumn2dThermalAI),
...
double
DispBeamColumn2dThermalAI::getPredictedStiffness(double Temperature, double deflection)
{
    ...
    pArgs = PyTuple_New(2);
    PyTuple_SetItem(pArgs, 0, PyFloat_FromDouble(Temperature));
    PyTuple_SetItem(pArgs, 1, PyFloat_FromDouble(deflection));
...
double
DispBeamColumn2dThermalAI::getPredictedForce(double Temperature, double deflection)
{
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
    ...
    pModule = PyImport_ImportModule("Loadmodel_module");
    if (pModule == NULL) {
        opserr << "Failed to load the module." << endln;
    }
...
int
DispBeamColumn2dThermalAI::getNumExternalNodes() const
{
    return 2;
}

const ID&
DispBeamColumn2dThermalAI::getExternalNodes()
{
    return connectedExternalNodes;
}

Node**
DispBeamColumn2dThermalAI::getNodePtrs()
{
    return theNodes;
}

int
DispBeamColumn2dThermalAI::getNumDOF()
{
    return 6;
}

void
DispBeamColumn2dThermalAI::setDomain(Domain* theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
        opserr << "FATAL ERROR DispBeamColumn2dThermalAI (tag: %d), node not found in domain";
        //	this->getTag());
    ...
    if (dofNd1 != 3 || dofNd2 != 3) {
        opserr << "FATAL ERROR DispBeamColumn2dThermalAI (tag: %d), has differing number of DOFs at its nodes";
        //	this->getTag());
...
int
DispBeamColumn2dThermalAI::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
        opserr << "DispBeamColumn2dThermalAI::commitState () - failed in base class";
    }
...
int
DispBeamColumn2dThermalAI::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalAI::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumn2dThermalAI::update(void)
{
    int err = 0;
...
//opserr << "BeamNUT_" << this->getTag() << " ,  v  " << v << endln;
    ...
    if (err != 0) {
        opserr << "DispBeamColumn2dThermalAI::update() - failed setTrialSectionDeformations()\n";
        return err;
    }
...
const Matrix&
DispBeamColumn2dThermalAI::getTangentStiff()
{
    ...
    //    K = K;
    //}
    K(1, 1) = getPredictedStiffness(temp, disp);
...
const Matrix&
DispBeamColumn2dThermalAI::getInitialBasicStiff()
{
    static Matrix kb(3, 3);
...
const Matrix&
DispBeamColumn2dThermalAI::getInitialStiff()
{
...
const Matrix&
DispBeamColumn2dThermalAI::getMass()
{
    K.Zero();
...
void
DispBeamColumn2dThermalAI::zeroLoad(void)
{
    Q.Zero();
...
int
DispBeamColumn2dThermalAI::addLoad(ElementalLoad* theLoad, double loadFactor)
{
    int type;
...
#ifdef _BDEBUG
            opserr << "DispBeamColumn2dThermalAI::addLoad at the ele " << this->getTag() << " section " << i << " --theIntCrds" << theIntCrds << endln;
#endif
    ...
    //----------------------------------------
        ...
        opserr << "DispBeamColumn2dThermalAI::addLoad(double) -- load type " << theLoad->getClassType()
...
int
DispBeamColumn2dThermalAI::addLoad(ElementalLoad* theLoad, const Vector& factors)
{
    int type;
   ...
   //JZ 07/10 /////////////////////////////////////////////////////////////start
    if (type == LOAD_TAG_Beam2dThermalAction) {
        ...
        opserr << "DispBeamColumn2dThermalAI::addLoad(Vector) -- load type " << theLoad->getClassType()
            << "unknown for element with tag: " << this->getTag() << "\n";
        return -1;
    }
...
int
DispBeamColumn2dThermalAI::addInertiaLoadToUnbalance(const Vector& accel)
{
    ...
    if (3 != Raccel1.Size() || 3 != Raccel2.Size()) {
        opserr << "DispBeamColumn2dThermalAI::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
        return -1;
    }
...
const Vector&
DispBeamColumn2dThermalAI::getResistingForce()
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
const Vector&
DispBeamColumn2dThermalAI::getResistingForceIncInertia()
{
...
int
DispBeamColumn2dThermalAI::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn2dThermalAI::sendSelf() - failed to send ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn2dThermalAI::sendSelf() - failed to send double data\n";
            return -1;
        }
    }
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn2dThermalAI::sendSelf() - failed to send crdTranf\n";
        return -1;
    }
    ...
    for (j = 0; j < numSections; j++) {
        if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
            opserr << "DispBeamColumn2dThermalAI::sendSelf() - section " <<
...
int
DispBeamColumn2dThermalAI::recvSelf(int commitTag, Channel& theChannel,
    FEM_ObjectBroker& theBroker)
{
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn2dThermalAI::recvSelf() - failed to recv ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn2dThermalAI::sendSelf() - failed to recv double data\n";
            return -1;
        }
        alphaM = dData(0);
        betaK = dData(1);
    ...
    if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
        ...
        if (crdTransf == 0) {
            opserr << "DispBeamColumn2dThermalAI::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
    ...
    if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn2dThermalAI::sendSelf() - failed to recv crdTranf\n";
        return -3;
    }
    ...
    if (numSections != idData(3)) {
        ...
        if (theSections == 0) {
            opserr << "DispBeamColumn2dThermalAI::recvSelf() - out of memory creating sections array of size " <<
                idData(3) << endln;
            return -1;
        }
        ...
        for (i = 0; i < numSections; i++) {
            ...
            if (theSections[i] == 0) {
                opserr << "DispBeamColumn2dThermalAI::recvSelf() - Broker could not create Section of class type " <<
                    sectClassTag << endln;
                exit(-1);
            }
            theSections[i]->setDbTag(sectDbTag);
            if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "DispBeamColumn2dThermalAI::recvSelf() - section " << i << " failed to recv itself\n";
                return -1;
            }
        }
...
void
DispBeamColumn2dThermalAI::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nDispBeamColumn2dThermalAI, element id:  " << this->getTag() << endln;
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{";
        s << "\"name\": " << this->getTag() << ", ";
        s << "\"type\": \"DispBeamColumn2dThermalAI\", ";
...
int
DispBeamColumn2dThermalAI::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** displayModes, int numModes)
{
    static Vector v1(3);
    static Vector v2(3);
...
Response*
DispBeamColumn2dThermalAI::setResponse(const char** argv, int argc,
    OPS_Stream& output)
{
    Response* theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType", "DispBeamColumn2dThermalAI");
...
int
DispBeamColumn2dThermalAI::getResponse(int responseID, Information& eleInfo)
{
    double V;
    double L = crdTransf->getInitialLength();
...
int
DispBeamColumn2dThermalAI::setParameter(const char** argv, int argc, Parameter& param)
{
    if (argc < 1)
        return -1;
...
int
DispBeamColumn2dThermalAI::updateParameter(int parameterID, Information& info)
{
    if (parameterID == 1) {
        rho = info.theDouble;
        return 0;
...
int
DispBeamColumn2dThermalAI::activateParameter(int passedParameterID)
{
...
const Matrix&
DispBeamColumn2dThermalAI::getKiSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Matrix&
DispBeamColumn2dThermalAI::getMassSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Vector&
DispBeamColumn2dThermalAI::getResistingForceSensitivity(int gradNumber)
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
// NEW METHOD
int
DispBeamColumn2dThermalAI::commitSensitivity(int gradNumber, int numGrads)
{
