...
// $Source: /usr/local/cvs/OpenSees/SRC/element/dispBeamColumn/DispBeamColumn3dThermalAI.cpp,v $

// Written: MHS
// Created: Feb 2001
//
// Description: This file contains the class definition for DispBeamColumn3dThermalAI.

#include <DispBeamColumn3dThermalAI.h>
#include <Node.h>
...
#include <Python.h>
Matrix DispBeamColumn3dThermalAI::K(12, 12);
Vector DispBeamColumn3dThermalAI::P(12);
double DispBeamColumn3dThermalAI::workArea[200];

void* OPS_DispBeamColumn3dThermalAI()
{
    ...
    Element* theEle = new DispBeamColumn3dThermalAI(iData[0], iData[1], iData[2], secTags.Size(), sections,
        *bi, *theTransf, mass);
    delete[] sections;
    return theEle;
}

DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI(int tag, int nd1, int nd2,
    int numSec, SectionForceDeformation** s,
    BeamIntegration& bi,
    CrdTransf& coordTransf, double r)
    :Element(tag, ELE_TAG_DispBeamColumn3dThermalAI),
    ...
    Q(12), q(6), rho(r), parameterID(0)
{
    ...
    if (theSections == 0) {
        opserr << "DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI - failed to allocate section model pointer\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        // Check allocation
        if (theSections[i] == 0) {
            opserr << "DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI -- failed to get a copy of section model\n";
            exit(-1);
        }
    }
    ...
    if (beamInt == 0) {
        opserr << "DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI - failed to copy beam integration\n";
        exit(-1);
    }
    ...
    if (crdTransf == 0) {
        opserr << "DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI - failed to copy coordinate transformation\n";
        exit(-1);
    }
...
DispBeamColumn3dThermalAI::DispBeamColumn3dThermalAI()
    :Element(0, ELE_TAG_DispBeamColumn3dThermalAI),
...
double
DispBeamColumn3dThermalAI::getPredictedStiffness(double Temperature, double deflection)//int eleTag, 
{
...
double
DispBeamColumn3dThermalAI::getPredictedForce(double Temperature, double deflection) //int eleTag,
{ 
    PyObject* pModule, * pFunc;
    PyObject* pArgs, * pValue;
...
int
DispBeamColumn3dThermalAI::getNumExternalNodes() const
{
    return 2;
}
...
DispBeamColumn3dThermalAI::getExternalNodes()
{
    return connectedExternalNodes;
}

Node**
DispBeamColumn3dThermalAI::getNodePtrs()
{
...
int
DispBeamColumn3dThermalAI::getNumDOF()
{
    return 12;
}

void
DispBeamColumn3dThermalAI::setDomain(Domain* theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
        //opserr << "FATAL ERROR DispBeamColumn3dThermalAI (tag: %d), node not found in domain",
        //	this->getTag());
    ...
    if (dofNd1 != 6 || dofNd2 != 6) {
        //opserr << "FATAL ERROR DispBeamColumn3dThermalAI (tag: %d), has differing number of DOFs at its nodes",
        //	this->getTag());
...
int
DispBeamColumn3dThermalAI::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
        opserr << "DispBeamColumn3dThermalAI::commitState () - failed in base class";
    }
...
int
DispBeamColumn3dThermalAI::revertToLastCommit()
{
    int retVal = 0;
...
int
DispBeamColumn3dThermalAI::revertToStart()
{
    int retVal = 0;
...
int
DispBeamColumn3dThermalAI::update(void)
{
    int err = 0;
    ...
    if (err != 0) {
        opserr << "DispBeamColumn3dThermalAI::update() - failed setTrialSectionDeformations()\n";
        return err;
    }
    return 0;
}
...
const Matrix&
DispBeamColumn3dThermalAI::getTangentStiff()
{
    ...
    K(2, 2) = getPredictedStiffness(Temperature, deflection);
...
const Matrix&
DispBeamColumn3dThermalAI::getInitialBasicStiff()
{
    static Matrix kb(6, 6);
...
const Matrix&
DispBeamColumn3dThermalAI::getInitialStiff()
{
...
const Matrix&
DispBeamColumn3dThermalAI::getMass()
{
    K.Zero();
...
void
DispBeamColumn3dThermalAI::zeroLoad(void)
{
    Q.Zero();
...
int
DispBeamColumn3dThermalAI::addLoad(ElementalLoad* theLoad, double loadFactor)
{
    int type;
    ...
    //----------------------------------------------
        ...
        opserr << "DispBeamColumn3dThermalAI::addLoad() -- load type unknown for element with tag: " <<
            this->getTag() << endln;
        return -1;
    }
...
int
DispBeamColumn3dThermalAI::addInertiaLoadToUnbalance(const Vector& accel)
{
    ...
    if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
        opserr << "DispBeamColumn3dThermalAI::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
        return -1;
    }
...
const Vector&
DispBeamColumn3dThermalAI::getResistingForce()
{
    double L = crdTransf->getInitialLength();
    ...
    //opserr<<"Beam "<<q<<endln;
    P(2) = getPredictedForce(Temperature, deflection);
...
const Vector&
DispBeamColumn3dThermalAI::getResistingForceIncInertia()
{
...
int
DispBeamColumn3dThermalAI::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to send ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to send double data\n";
            return -1;
        }
    }
    ...
    if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to send crdTranf\n";
        return -1;
    }

    // send the beam integration
    if (beamInt->sendSelf(commitTag, theChannel) < 0) {
        opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to send beamInt\n";
        return -1;
    }
    ...
    for (j = 0; j < numSections; j++) {
        if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
            opserr << "DispBeamColumn3dThermalAI::sendSelf() - section " << j << "failed to send itself\n";
            return -1;
        }
    }
...
int
DispBeamColumn3dThermalAI::recvSelf(int commitTag, Channel& theChannel,
    FEM_ObjectBroker& theBroker)
{
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
        opserr << "DispBeamColumn3dThermalAI::recvSelf() - failed to recv ID data\n";
        return -1;
    }
    ...
    if (idData(6) == 1) {
        ...
        if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
            opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to recv double data\n";
            return -1;
        }
        alphaM = dData(0);
        betaK = dData(1);
    ...
    if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
        ...
        if (crdTransf == 0) {
            opserr << "DispBeamColumn3dThermalAI::recvSelf() - " <<
    ...
    if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to recv crdTranf\n";
        return -3;
    }
    ...
    if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
        ...
        if (beamInt == 0) {
            opserr << "DispBeamColumn3dThermalAI::recvSelf() - failed to obtain the beam integration object with classTag" <<
    ...
    {
        opserr << "DispBeamColumn3dThermalAI::sendSelf() - failed to recv beam integration\n";
        return -3;
    }
    ...
    if (numSections != idData(3)) {
        ...
        if (theSections == 0) {
            opserr << "DispBeamColumn3dThermalAI::recvSelf() - out of memory creating sections array of size" <<
                idData(3) << endln;
            exit(-1);
        }
        ...
        for (i = 0; i < numSections; i++) {
            ...
            if (theSections[i] == 0) {
                opserr << "DispBeamColumn3dThermalAI::recvSelf() - Broker could not create Section of class type" <<
                    sectClassTag << endln;
                exit(-1);
            }
            theSections[i]->setDbTag(sectDbTag);
            if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "DispBeamColumn3dThermalAI::recvSelf() - section " <<
...
void
DispBeamColumn3dThermalAI::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nDispBeamColumn3dThermalAI, element id:  " << this->getTag() << endln;
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{";
        s << "\"name\": " << this->getTag() << ", ";
        s << "\"type\": \"DispBeamColumn3dThermalAI\", ";
...
int
DispBeamColumn3dThermalAI::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** modes, int numModes)
{
    static Vector v1(3);
    static Vector v2(3);
...
Response*
DispBeamColumn3dThermalAI::setResponse(const char** argv, int argc, OPS_Stream& output)
{

    Response* theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType", "DispBeamColumn3dThermalAI");
...
int
DispBeamColumn3dThermalAI::getResponse(int responseID, Information& eleInfo)
{
    double N, V, M1, M2, T;
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
int
DispBeamColumn3dThermalAI::setParameter(const char** argv, int argc, Parameter& param)
{
    if (argc < 1)
        return -1;
...
int
DispBeamColumn3dThermalAI::updateParameter(int parameterID, Information& info)
{
    if (parameterID == 1) {
        rho = info.theDouble;
        return 0;
...
int
DispBeamColumn3dThermalAI::activateParameter(int passedParameterID)
{
...
const Matrix&
DispBeamColumn3dThermalAI::getKiSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Matrix&
DispBeamColumn3dThermalAI::getMassSensitivity(int gradNumber)
{
    K.Zero();
    return K;
}
...
const Vector&
DispBeamColumn3dThermalAI::getResistingForceSensitivity(int gradNumber)
{
    double L = crdTransf->getInitialLength();
    double oneOverL = 1.0 / L;
...
// NEW METHOD
int
DispBeamColumn3dThermalAI::commitSensitivity(int gradNumber, int numGrads)
{
