...
Vector *ForceBeamColumn3dThermal::vsSubdivide = 0;
Matrix *ForceBeamColumn3dThermal::fsSubdivide = 0;
Vector *ForceBeamColumn3dThermal::SsrSubdivide = 0;
...
ForceBeamColumn3dThermal::ForceBeamColumn3dThermal(): 
  ...
  fs(0), vs(0), Ssr(0), vscommit(0), sp(0), Ki(0), isTorsion(false),Vsth0(0)
{
  theNodes[0] = 0;  
  theNodes[1] = 0;
  ...
  if (!vsSubdivide || !fsSubdivide || !SsrSubdivide) {
    opserr << "ForceBeamColumn3dThermal::ForceBeamColumn3dThermal() -- failed to allocate Subdivide arrays";   
    exit(-1);
  }
}
...
ForceBeamColumn3dThermal::setDomain(Domain *theDomain)
{
  ...
  if (L == 0.0) {
    opserr << "ForceBeamColumn3dThermal::setDomain(): Zero element length:" << this->getTag();  
    exit(0);
  }
...
ForceBeamColumn3dThermal::getInitialStiff(void)
{
  ...
  if (f.Solve(I, kvInit) < 0)
    opserr << "ForceBeamColumn3dThermal::getInitialStiff() -- could not invert flexibility";
	...
	if (initialFlag != 2) {
	  ...
	  for (j=0; j <numIters; j++) {
	    ...
	    if (f.Solve(I, kvTrial) < 0)
	      opserr << "ForceBeamColumn3dThermal::update() -- could not invert flexibility\n";
  ...
  ForceBeamColumn3dThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
  {
    ...
    if (sp == 0) {
      ...
      if (sp == 0) {
	opserr << "ForceBeamColumn3dThermal::addLoad -- out of memory\n";
	exit(-1);
      }
    }
...
ForceBeamColumn3dThermal::getInitialDeformations(Vector &v0)
{
  ...
  int
  ForceBeamColumn3dThermal::displaySelf(Renderer &theViewer, int displayMode, float fact)
  {
...
ForceBeamColumn3dThermal::updateParameter (int parameterID, Information &info)
{
  ...
  if (parameterID == 1) {
    ...
    if (ok < 0) {
      opserr << "ForceBeamColumn3dThermal::updateParameter() - could not update parameter. " << endln;
      return ok;
    }
    else {
      return ok;
