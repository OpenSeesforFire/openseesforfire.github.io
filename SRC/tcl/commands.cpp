...
int wipeSIFBuilder(ClientData, Tcl_Interp *, int, TCL_Char **);
...
int OpenSeesAppInit(Tcl_Interp *interp) {
  ...
  theSimulationInfoPtr = &simulationInfo;
	...
	//Added by Liming for SIFBUILDER PROJECT, UoE,2014

	Tcl_CreateCommand(interp,"SIFBuilder", SIFBuilder,
				(ClientData)NULL, (Tcl_CmdDeleteProc *) NULL);
  Tcl_CreateCommand(interp,"wipeSIF", wipeSIFBuilder,
                    (ClientData)NULL, (Tcl_CmdDeleteProc *) NULL);
#endif
...
//Added by UoE group for using SIFBuilder,2014
#ifdef _SIFBUILDER
int
SIFBuilder(ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char **argv)
{

	 if (theTclSIFBuilder == 0) {
		int DisplayIndex=0;
		//identifying the parameters following "SIFBuilder" command
		if (argc>1) {
			if ((strcmp(argv[1], "-display") == 0) || (strcmp(argv[1], "display") == 0) || (strcmp(argv[1], "Display") == 0)) {
				if(argc==2)
					DisplayIndex = 1;
				else if (argc == 3) {
					if (Tcl_GetInt(interp, argv[2], &DisplayIndex) != TCL_OK) {
						opserr << "WARNING:: invalid display index " << argv[2] << " for SIFBuilder \n";
						return TCL_ERROR;
					}
				}
				...
				opserr << "WARNING: SIFBuilder recieves an invalid argument" << endln;

		}
		//end of argc>1

		theTclSIFBuilder = new TclSIFBuilder(theDomain, interp, DisplayIndex);
...
int
wipeSIFBuilder(ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char **argv)
{
  if (theTclSIFBuilder != 0) {
