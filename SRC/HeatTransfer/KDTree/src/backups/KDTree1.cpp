...
int KDTree::range_search(int nodeIdx, Range * range, int dim)
{
  ...
  double x1 = range[0][0]; //0.45
  double x2 = range[0][1]; //0.55
  double y1 = range[1][0]; //0.25
  double y2 = range[1][1]; //0.3499
...
//int KDTree::closest_point(double *pnt, int &idx, bool approx)
//{
//  int dim = 0;
//
...
//
//  double ndistsq = distsq(pnt,points+idx*ndim);
//	
...
//				  double *pnt, double &cdistsq, int &idx)
//{
...
//	struct _Node *node = nodeMem+nodeIdx;
//  if (node->pntidx >= 0) {
//    double dsq = distsq(pnt, points+node->pntidx*ndim);
//    if (dsq < cdistsq) {
//      cdistsq = dsq;
//      idx = node->pntidx;
//    }
...
//
//
//inline double KDTree::distsq(double *pnt1, double *pnt2)
//{
//  double d = 0.0;
//  for (int i = 0; i < ndim; i++)
...
int main(int argc, char *argv[])
{
  ...
  double *data = new double[npoints*ndim];
  ...
  for(int i=0;i<1000;i++) 
    for(int j=0;j<3;j++)
       pnts[i][j] = (double)rand()/(double)RAND_MAX;
